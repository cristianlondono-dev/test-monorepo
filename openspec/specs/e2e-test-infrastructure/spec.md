...
TBD - Defines the tooling that makes end-to-end browser testing possible for this project: the Playwright MCP server registration for exploratory Claude Code sessions, and the `@playwright/test` setup that runs the scripted regression suite against `apps/web` with its own data lifecycle.
